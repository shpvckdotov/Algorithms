...
void naive_tree_timetest(FILE* output) {
    for (int size = STRT_INSERT_NUM; size < MAX_INSERT_NUM + 1; size += DELTA_INSERT_NUM) {
        ...
        for (int iter = 0; iter < CALC_NUM; ++iter) {
            NodeN* rand_tree = naive_tree_ctor();
            NodeN* sort_tree = naive_tree_ctor();

            Container cur_values = naive_tree_timetest1(rand_tree, sort_tree, size, output);
...
Container naive_tree_timetest1(NodeN* rand_tree, NodeN* sort_tree, int insert_num, FILE* output) {
    ...
    test_values.insert_time = naive_tree_insert_test(rand_tree, insert_num, rand_arg_arr);
    test_values.remove_time = naive_tree_remove_test(rand_tree, insert_num / 2, insert_num, rand_arg_arr);
    test_values.insert_sort_time = naive_tree_sort_insert_test(sort_tree, insert_num, sort_arg_arr);
    test_values.remove_sort_time = naive_tree_sort_remove_test(sort_tree, insert_num / 2, insert_num, sort_arg_arr);
...
double naive_tree_sort_insert_test(NodeN* tree, int insert_num, int* arg_arr) {
...
double naive_tree_sort_remove_test(NodeN* tree, int remove_num, int arr_size, int* arg_arr) {
    double time = 0;
...
double naive_tree_insert_test(NodeN* tree, int insert_num, int* arg_arr) {
    double time = 0;
...
double naive_tree_remove_test(NodeN* tree, int remove_num, int arr_size, int* arg_arr) {
...
int main() {
    FILE* output = fopen("naive_output.csv", "w+");
    naive_tree_timetest(output);
    fclose(output);
}
