...
int getMin(int** sparseTable, int n, int* degs, int l, int r){    int lo = degs[r - l + 1];
    return min(sparseTable[lo][l], sparseTable[lo][r - (1 << lo) + 1]);
}


int main(){
    FILE* file = fopen("sparse1.txt", "w+");
    int* degs = (int*)malloc((SIZE_ARR + 1) * sizeof(int));
    buildDegs(degs, SIZE_ARR);
    int* a = (int*)malloc((SIZE_ARR + 1) * sizeof(int));
    int** sparseTable = (int**)calloc(degs[SIZE_ARR] + 1, sizeof(int*));
    for (int i = 0; i <= degs[SIZE_ARR]; i++)
    {
        sparseTable[i] = (int*)calloc(SIZE_ARR + 1, sizeof(int));
    }
    long time = 0;
    for (int i = 0; i < N_TEST; i++) {
        ...
        buildST(sparseTable, a, SIZE_ARR, degs);
        ...
        for (int j = 0; j < SIZE_ARR; j++){
            ...
            int r = l + len;
            long long min = getMin(sparseTable, SIZE_ARR, degs, l, r);
