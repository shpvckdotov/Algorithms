...
void decart_tree_timetest(FILE* output) {
    for (int size = STRT_INSERT_NUM; size < MAX_INSERT_NUM + 1; size += DELTA_INSERT_NUM) {
        ...
        for (int iter = 0; iter < CALC_NUM; ++iter) {
            NodeD* rand_tree = decart_tree_ctor();
            NodeD* sort_tree = decart_tree_ctor();

            Container cur_values = decart_tree_timetest1(rand_tree, sort_tree, size, output);
...
Container decart_tree_timetest1(NodeD* rand_tree, NodeD* sort_tree, int insert_num, FILE* output) {
    ...
    test_values.insert_time = decart_tree_insert_test(rand_tree, insert_num, rand_arg_arr);
    test_values.remove_time = decart_tree_remove_test(rand_tree, insert_num / 2, insert_num, rand_arg_arr);
    test_values.insert_sort_time = decart_tree_sort_insert_test(sort_tree, insert_num, sort_arg_arr);
    test_values.remove_sort_time = decart_tree_sort_remove_test(sort_tree, insert_num / 2, insert_num, sort_arg_arr);
...
double decart_tree_sort_insert_test(NodeD* tree, int insert_num, int* arg_arr) {
...
double decart_tree_sort_remove_test(NodeD* tree, int remove_num, int arr_size, int* arg_arr) {
    double time = 0;
...
double decart_tree_insert_test(NodeD* tree, int insert_num, int* arg_arr) {
    double time = 0;
...
double decart_tree_remove_test(NodeD* tree, int remove_num, int arr_size, int* arg_arr) {
...
int main() {
    FILE* output = fopen("decart_output.csv", "w+");
    decart_tree_timetest(output);
    fclose(output);
}
