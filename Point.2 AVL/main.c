...
void avl_tree_timetest(FILE* output) {
    for (int size = STRT_INSERT_NUM; size < MAX_INSERT_NUM + 1; size += DELTA_INSERT_NUM) {
        ...
        for (int iter = 0; iter < CALC_NUM; ++iter) {
            avlNode* rand_tree = avl_tree_ctor();
            avlNode* sort_tree = avl_tree_ctor();

            Container cur_values = avl_tree_timetest1(rand_tree, sort_tree, size, output);
...
Container avl_tree_timetest1(avlNode* rand_tree, avlNode* sort_tree, int insert_num, FILE* output) {
...
double avl_tree_sort_insert_test(avlNode* tree, int insert_num, int* arg_arr) {
...
double avl_tree_sort_remove_test(avlNode* tree, int remove_num, int arr_size, int* arg_arr) {
    double time = 0;

    for (int remove_iter = 0; remove_iter < remove_num; ++remove_iter)
    {
        double remove_start = clock();
        tree = eraseAvl(tree, arg_arr[remove_iter]);
        double remove_end = clock();
...
double avl_tree_insert_test(avlNode* tree, int insert_num, int* arg_arr) {
    double time = 0;
    ...
    for (int insert_iter = 0; insert_iter < insert_num; ++insert_iter)
    {
        ...
        double insert_start = clock();
        tree = insertAvl(tree, arg);
        double insert_end = clock();
...
double avl_tree_remove_test(avlNode* tree, int remove_num, int arr_size, int* arg_arr) {
...
int main() {
    FILE* output = fopen("avl_output.csv", "w+");
