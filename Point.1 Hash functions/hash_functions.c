...
#define INIT_HASH 0xA2F58F47FDCF22D5
#define MULT 912'784'669'717ul
#define MULT_ADD 735'228'619'038ul

long long HashMultiplicative(int key) {
    return ((long long) key) * MULT + MULT_ADD;
}
...
long long HashPolynomial(char* string) {
    long long hash = INIT_HASH;
    while (*string) {
        hash = hash * MULT + (long long) *(string++);
    }
    return hash;
}
