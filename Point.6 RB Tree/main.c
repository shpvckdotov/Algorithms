...
void red_black_tree_timetest(FILE* output) {
    for (int size = STRT_INSERT_NUM; size < MAX_INSERT_NUM + 1; size += DELTA_INSERT_NUM) {
        ...
        for (int iter = 0; iter < CALC_NUM; ++iter) {
            avlNode* rand_tree = red_black_tree_ctor();
            avlNode* sort_tree = red_black_tree_ctor();

            Container cur_values = red_black_tree_timetest1(rand_tree, sort_tree, size, output);
...
Container red_black_tree_timetest1(avlNode* rand_tree, avlNode* sort_tree, int insert_num, FILE* output) {
    ...
    test_values.insert_time = red_black_tree_insert_test(rand_tree, insert_num, rand_arg_arr);
    test_values.remove_time = red_black_tree_remove_test(rand_tree, insert_num / 2, insert_num, rand_arg_arr);
    test_values.insert_sort_time = red_black_tree_sort_insert_test(sort_tree, insert_num, sort_arg_arr);
    test_values.remove_sort_time = red_black_tree_sort_remove_test(sort_tree, insert_num / 2, insert_num, sort_arg_arr);
...
double red_black_tree_sort_insert_test(avlNode* tree, int insert_num, int* arg_arr) {
...
double red_black_tree_sort_remove_test(avlNode* tree, int remove_num, int arr_size, int* arg_arr) {
    double time = 0;
...
double red_black_tree_insert_test(avlNode* tree, int insert_num, int* arg_arr) {
    double time = 0;
...
double red_black_tree_remove_test(avlNode* tree, int remove_num, int arr_size, int* arg_arr) {
...
int main() {
    FILE* output = fopen("rb_output.csv", "w+");
    red_black_tree_timetest(output);
    fclose(output);
}
